...
- (UIImage *)lowerHandleImageNormal
{
    ...
    return nil; // Modified by wanchenxie.
...
//     NSLog(@"图片高度：%f", thumbImage.size.height);
//     NSLog(@"Slider高度：%f", thumbRect.size.height);
//    NSLog(@"高度：%f", self.bounds.size.height);
...
-(void)layoutSubviews
{
    ...
    //self.lowerHandle.userInteractionEnabled = NO;   // Added by wanchenxie.
...
//  lowerHandle was commented by wanchenxie.
...
//    }
    ...
    if(CGRectContainsPoint(UIEdgeInsetsInsetRect(_upperHandle.frame, self.upperTouchEdgeInsets), touchPoint))
    {
        ...
        if (self.delegate != nil && [self.delegate respondsToSelector:@selector(rangeSlider:begineChangedWithEvent:)]) {
            [self.delegate performSelector:@selector(rangeSlider:begineChangedWithEvent:) withObject:self withObject:event];
        }
    }
...
//    }
    ...
    if(_upperHandle.highlighted )
    {
        ...
        if (self.delegate != nil && [self.delegate respondsToSelector:@selector(rangeSlider:continueChangeWithEvent:)]) {
            [self.delegate performSelector:@selector(rangeSlider:continueChangeWithEvent:) withObject:self withObject:event];
        }
    }
...
-(void)endTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    if(_upperHandle.highlighted ) {
        ...
        if (self.delegate != nil && [self.delegate respondsToSelector:@selector(rangeSlider:endChangeWithEvent:)]) {
            [self.delegate performSelector:@selector(rangeSlider:endChangeWithEvent:) withObject:self withObject:event];
        }
